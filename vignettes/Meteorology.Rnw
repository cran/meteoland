...
\end{equation}
To correct minimum (or maximum) temperatures bias correction is applied to the difference between minimum (or maximum) temperature and mean temperature. We will describe the process for minimum temperature and maximum temperature is analogous. First, we define $x$ and $u$ as the difference in temperature ($\delta$):
\begin{eqnarray}
x &=& \delta_{min, obs} = T_{min, obs} - T_{mean, obs}\\
u &=& \delta_{min, est} = T_{min, est} - T_{mean, est}
\end{eqnarray}
Then, we calculate monthly biases $\theta_m$ as before. Correction is performed on the difference variable, i.e.:
\begin{equation}
\delta_{min, cor} = \max \{0,\, \delta_{min, est} - \theta_{m(i)} \}
\end{equation}
where the maximum function is used to prevent the corrected difference from becoming positive (negative in the case of maximum temperatures). Finally, the corrected difference is added to the corrected mean temperature to obtain the corrected minimum temperature:
\begin{equation}
T_{min, cor} = T_{mean, cor} + \delta_{min, cor}
\end{equation}
...
Radiation and wind speed are corrected using the general bias correction procedure used for mean temperature. In the case of relative humidity, values are transformed to specific humidity, bias correction is applied to this variable and the result is back-transformed to relative humidity. Since historic wind data is often not available, if wind speed data is missing the coarse-scale wind estimate is taken directly without correction.
