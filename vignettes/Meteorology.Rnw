...
%\VignetteIndexEntry{Package meteoland}
...
\title{Package meteoland (ver. \Sexpr{packageVersion("meteoland")})}
...
With the aim to assist research of climatic impacts on forests, the R package \texttt{meteoland} (De Cáceres et al. 2018) provides utilities to estimate daily weather variables at any position over complex terrains:
\begin{enumerate}
...
\subsection{Visualizing input topography and meteorological data}
...
\end{itemize}
Similarly, function \texttt{spplot()} also accepts objects of classes \texttt{SpatialGridTopography} and \texttt{SpatialPixelsTopography}, so that topography can also be easily displayed.

\subsection{Manipulating topographic and meteorological data}
Following the design of package \texttt{sp}, package meteoland provides functions for switching between spatial structures. For example, it is possible to coerce an object of \texttt{SpatialPointsTopography} into a \texttt{SpatialPixelsTopography} using a call like \texttt{as(x, "SpatialPixelsTopography")}. Subsetting is also allowed, so that estimations of daily meteorology can be performed in subsets of points or grids.
...
\subsection{Summarizing meteorological data}
...
\subsection{Downloading data from station networks}

National meteorological agencies are increasingly adopting an open data philosophy and two of them are the Spanish meteorological bureau (Agencia Española de Meteorología, AEMET) and the Catalan meteorology service (Servei Català de Meteorologia, SMC). In \texttt{meteoland} we provide functions that retrieve AEMET and SMC data using their OpenData API (API keys need to be obtained from AEMET or SMC):
\begin{itemize}
\item{\texttt{downloadAEMEThistoricalstationlist()} : Gets the list of AEMET stations from which historical daily meteorological data is available.}
\item{\texttt{downloadAEMEThistorical()} : Downloads historical daily meteorological data corresponding to a input set of AEMET stations and a given period.}
\item{\texttt{downloadAEMETcurrentday()} : Downloads the last 24h of meteorological data corresponding to a input set of AEMET stations.}
\item{\texttt{downloadSMCvarmetadata()}: Downloads the definition of meteorological variables, their units and codes for SMC.}
\item{\texttt{downloadSMCcurrentday()} : Downloads the last 24h of meteorological data corresponding to a input set of SMC stations.}
\item{\texttt{downloadSMChistorical()} : Downloads historical daily meteorological data corresponding to a input set of SMC stations and a given period.}
\end{itemize}

\subsection{Reshaping data obtained from other packages}
\texttt{meteoland} provides functions to facilitate reshaping weather data acquired using other R packages into the meteoland format. So far, two packages are supported, \texttt{worldmet} and \texttt{weathercan}, and the corresponding functions are called \texttt{reshapeworldmet()} and \texttt{reshapeweathercan()}, respectively.
...
\section{References}
\begin{itemize}
\item{Boé J, Terray L, Habets F, Martin E (2007) Statistical and dynamical downscaling of the Seine basin climate for hydro-meteorological studies. Int J Climatol 27:1643–1655. doi: 10.1002/joc.1602}

\item{Danby, J. M. Eqn. 6.16.4 in Fundamentals of Celestial Mechanics, 2nd ed. Richmond, VA: Willmann-Bell, p. 207, 1988.}

\item{Déqué M (2007) Frequency of precipitation and temperature extremes over France in an anthropogenic scenario: Model results and statistical correction according to observed values. Glob Planet Change 57:16–26. doi: 10.1016/j.gloplacha.2006.11.030}

\item{De Caceres M, Martin-StPaul N, Turco M, Cabon A, Granda V (2018) Estimating daily meteorological data and downscaling climate models over landscapes. Environmental Modelling and Software 108: 186-196.}
