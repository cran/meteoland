...
\item{Function \texttt{correctiongrid()} [normally not used] performs statistical correction of weather data series over a grid and it returns an object of class \texttt{SpatialPointsMeteorology} containing corrected weather predictions.}
\end{itemize}
